...
namespace MVCPlayWithMe.OpenPlatform.API.TikiAPI.Order
{
    ...
    class TikiGetListOrders
    {
        ...
        static public List<MVCPlayWithMe.OpenPlatform.Model.TikiApp.Order.TikiOrder> GetListOrderAShop(TikiConfigApp configApp, EnumOrderItemFilterByDate interval)
        {
            ...
            while (true)
            {
                if (currentPage == 1)// lấy page đầu
                {
                    ...
                    listValuePair.Add(new DevNameValuePair(ArrayStringQueryParameters[(Int32)EnumQueryParameters.item_inventory_type], TikiOrderItemInventoryType.ArrayStringOrderItemInventoryType[(int)TikiOrderItemInventoryType.EnumOrderItemInventoryType.backorder]));
                    ...
                    if(interval == EnumOrderItemFilterByDate.today)
                        listValuePair.Add(new DevNameValuePair(ArrayStringQueryParameters[(Int32)EnumQueryParameters.filter_date_by], TikiOrderItemFilterByDate.ArrayStringOrderItemFilterByDate[(Int32)TikiOrderItemFilterByDate.EnumOrderItemFilterByDate.today]));
                ...
                try
                {
                    ...
                    TikiPageOrders pageOrders = JsonConvert.DeserializeObject<TikiPageOrders>(json, settings);
                    lsOrder.AddRange(pageOrders.data);
                    if (currentPage == pageOrders.paging.last_page // Đã lấy page đơn hàng cuối cùng
                        || pageOrders.paging.last_page == 0)// Không có đơn hàng nào nên last_page = 0
                        break;
                    else
                        currentPage++;
                }
        ...
        /// </summary>
        /// <param name="lsOrderTikiFullInfo"></param>
        /// <returns></returns>
        public static Dictionary<string, int> TikiGetDictionaryOfProductQuantityFromListDetailOrder(List<MVCPlayWithMe.OpenPlatform.Model.TikiApp.Order.TikiOrder> lsOrderTikiFullInfo)
        {
            return null;
            //if (lsOrderTikiFullInfo == null || lsOrderTikiFullInfo.Count() == 0)
            //    return null;
            ...
            //string orderIdTemp = string.Empty;
            //foreach (var eDetail in lsOrderTikiFullInfo)
            //{
            //    foreach (var e in eDetail.items)
            //    {
            //        orderIdTemp = e.id.ToString();
            //        if (lsProdtuctOnTMDT.ContainsKey(orderIdTemp))
            //        {
            //            lsProdtuctOnTMDT[orderIdTemp] = lsProdtuctOnTMDT[orderIdTemp] + e.qty;
            //        }
            //        else
            //            lsProdtuctOnTMDT.Add(orderIdTemp, e.qty);
            //    }
            //}
            ...
            //string productIdTemp = string.Empty;
            //foreach (KeyValuePair<string, int> ite in lsProdtuctOnTMDT)
            //{
            //    List<ModelMappingSanPhamTMDT_SanPhamKho> ls = null;
            //    ls = ModelMappingSanPhamTMDT_SanPhamKho.Tiki_GetListModelMappingSanPhamTMDT_SanPhamKhoFromIDProTMDT(ite.Key);
            //    foreach (ModelMappingSanPhamTMDT_SanPhamKho obj in ls)
            //    {
            //        productIdTemp = obj.code;
            //        if (lsProdtuctInWarehouse.ContainsKey(productIdTemp))
            //        {
            //            lsProdtuctInWarehouse[productIdTemp] = lsProdtuctInWarehouse[productIdTemp] + ite.Value * Common.ConvertStringToInt32(obj.quantity);
            //        }
            //        else
            //        {
            //            lsProdtuctInWarehouse.Add(productIdTemp, ite.Value * Common.ConvertStringToInt32(obj.quantity));
            //        }
            //    }
